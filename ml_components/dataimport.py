...
def get_import_query(
    ...
    #     ) AS table_name,
    #     row_count,
    # FROM `airasia-etlbq.BigQueryCost.ALL_PROJECT_TABLE_LIST_*`
    ...
    SELECT
        ...
        WHERE snapshot_date BETWEEN '{start_date}' AND '{end_date}'
        and (
          (project_id = 'radiant-micron-790' and lower(dataset_id) = 'ods' and lower(table_id) = 'booking') or  -- sure got data drift row keep increasing
          (project_id = 'radiant-micron-790' and lower(dataset_id) = 'master' and lower(table_id) = 'mastersquare') or  -- sure got data drift row keep increasing
          ----------------
          (project_id = 'radiant-micron-790' and lower(dataset_id) = 'diqit' and lower(table_id) = 'order__order_20210510') or  -- sure got data drift but got idle now
          (project_id = 'radiant-micron-790' and lower(dataset_id) = 'diqit' and lower(table_id) = 'order__order_detail_20210510') or  -- sure got data drift but got idle now
          ----------------
          (project_id = 'airasia-etlbq' and lower(dataset_id) = 'rokki' and lower(table_id) = 'flight') or  -- no data drift idle
          (project_id = 'airasia-common-reporting' and lower(dataset_id) = 'aachat' and lower(table_id) = 'chatfunnel')   -- no data drift idle
        )
    """
