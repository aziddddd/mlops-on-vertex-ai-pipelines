...
def grand_drift_check(
    ...
    if len(dataset_q_df) > 0 and len(dataset_p_df) > 0:
        ...
        html = '<br>' + f'<h2>{sku_column} : {sku_name}</h2>'
        ...
        datadrift_html_save_path = f'datadrift_{sku_name}.html'
        with open(datadrift_html_save_path, 'w') as f:
            f.write(html)
        ...
        datadrift_html_artifact_path = f'artifacts/html/datadrift/{src_run_dt}/'

        blob = bucket.blob(datadrift_html_artifact_path + datadrift_html_save_path)
        blob.upload_from_filename(datadrift_html_save_path)
