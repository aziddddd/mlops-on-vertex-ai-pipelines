...
def get_period(
    ...
    # train_start_date = datetime.strftime(run_date_obj - timedelta(days=1), '%Y-%m-%d')
    # train_end_date = datetime.strftime(run_date_obj - timedelta(days=1), '%Y-%m-%d')
    train_start_date = run_date
    train_end_date = run_date
    pred_start_date = run_date
    pred_end_date = run_date

    print("Train starts on: ", train_start_date)
    print("Train ends on: ", train_end_date)
    print("Predict starts on: ", pred_start_date)
    print("Predict ends on: ", pred_end_date)
...
def get_import_query(
    ...
    catalog_query = f"""
    SELECT movie_title FROM `your-project-id.TEMP.movielens_movies_catalog`
    """
    ...
    SELECT
        ...
        user_rating,
        timestamp,
    FROM `your-project-id.TEMP.movielens_movies_ratings`
